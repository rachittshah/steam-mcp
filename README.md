...
```bash
STEAM_API_KEY=xxxxxxxx uvx steam-mcp
```

Or install it:

```bash
uv pip install steam-mcp   # or: pip install steam-mcp
```
...
Point the MCP server command at `uvx steam-mcp` (or `steam-mcp` if installed) and set
`STEAM_API_KEY` in the environment. See [`docs/clients.md`](./docs/clients.md) for
